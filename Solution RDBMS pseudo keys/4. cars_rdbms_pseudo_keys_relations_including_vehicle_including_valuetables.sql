...
CREATE TABLE `trim`(
  ...
  PRIMARY KEY (`trim_id`),
  INDEX `fk_trim_model1_idx` (`model_id` ASC),
  CONSTRAINT `fk_trim_model1` 
...
CREATE TABLE `engine`(
  ...
  INDEX `fk_engine_model1_idx` (`model_id` ASC),
  CONSTRAINT `fk_engine_model1`
    FOREIGN KEY (`model_id`) REFERENCES `cars_rdbms_pseudo_keys`.`model` (`model_id`) 
)ENGINE = InnoDB DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE `color`(
  ...
  INDEX `fk_color_make1_idx` (`make_id` ASC),
  CONSTRAINT `fk_color_make1` FOREIGN KEY (`make_id`) REFERENCES `cars_rdbms_pseudo_keys`.`make` (`make_id`)
)ENGINE = InnoDB DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE `vehicles` (
  ...
  PRIMARY KEY (`vin`),
  INDEX `fk_vehicles_make1_idx` (`make_id` ASC),
  INDEX `fk_vehicles_model1_idx` (`model_id` ASC),
  INDEX `fk_vehicles_engine1_idx` (`engine_id` ASC),
  INDEX `fk_vehicles_color1_idx` (`color_id` ASC),
  INDEX `fk_vehicles_transmission1_idx` (`transmission_id` ASC),
  INDEX `fk_vehicles_bodystyle1_idx` (`bodystyle_id` ASC),
  ...
  CONSTRAINT `fk_vehicles_transmission1` 
	FOREIGN KEY (`transmission_id`) REFERENCES `cars_rdbms_pseudo_keys`.`transmission` (`transmission_id`),
  CONSTRAINT `fk_vehicles_bodystyle1`
    FOREIGN KEY (`bodystyle_id`)  REFERENCES `cars_rdbms_pseudo_keys`.`bodystyle` (`bodystyle_id`)
